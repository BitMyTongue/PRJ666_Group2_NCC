...
export default function CreateTradeOffer() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (!res.ok)
          throw new Error(data?.error || "Failed to load trade offer");
  ...
  const requestItems = Array.isArray(listing?.requestItems)
    ? listing.requestItems
    : [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      if (!token) {
        throw new Error("Could not make listing: Session has expired");
      }
      ...
      // Success
      router.push(
        `/tradeOffers/create?listingId=${listingId}&status=true&id=${data.tradeOffer._id}`,
      );
    } catch (err) {
  ...
  if (status === "true") {
    ...
    const offerItems = Array.isArray(createdOffer.proposedItems)
      ? createdOffer.proposedItems
      : [];
        ...
        <div className="container-sm border border-gray rounded-4 shadow mt-7 mb-3 px-5 py-4">
          ...
          <div className="d-flex flex-column flex-md-row justify-content-center align-item-center">
            <div className="col-12 col-md-3 text-center text-md-start">
              <Image
                src={
                  listing?.images?.[0] || "/images/white-square-photo-frame.png"
                }
            ...
            <div
              className={`col-12 ${createdOffer?.meetUp ? "col-md-5" : "col-md-8"} mt-4 mt-md-0 pe-md-5`}
            >
            ...
            {createdOffer?.meetUp && (
              <div className="col-12 col-md-4 mt-4 my-md-0 mb-5">
                <p className="fw-semibold fs-5 text-primary">
                  Meet up location
                </p>
                {isLoaded && selectedMeetUpLocation && (
                  ...
                  >
                    <MarkerF
                      ...
                      onClick={() =>
                        setSelectedLocation(selectedMeetUpLocation)
                      }
                    />
          ...
          <form onSubmit={handleSubmit}>
            <div className="row mb-3">
              ...
              <div className="form-group col-md-6 col-lg-5 col-12 mb-4">
                <p className="text-primary mt-4 mb-4 fw-bold fs-5">
                  Proposed Money
                </p>
                <input
                  type="number"
